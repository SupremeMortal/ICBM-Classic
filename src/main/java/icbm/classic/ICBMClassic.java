...
import icbm.classic.config.IcbmClientConfig;
import icbm.classic.config.IcbmServerConfig;
...
public class ICBMClassic {
    ...
    private static final ModConfigSpec.Builder SERVER_CONFIG_SPEC = new ModConfigSpec.Builder();
    public static final IcbmServerConfig CONFIG = new IcbmServerConfig(SERVER_CONFIG_SPEC);
    private static final ModConfigSpec.Builder CLIENT_CONFIG_SPEC = new ModConfigSpec.Builder();
    public static final IcbmClientConfig CLIENT_CONFIG = new IcbmClientConfig(CLIENT_CONFIG_SPEC);
    ...
    public ICBMClassic(IEventBus bus) {
        ...
        if (FMLEnvironment.dist.isClient()) {
            ctx.registerConfig(ModConfig.Type.CLIENT, CLIENT_CONFIG_SPEC.build());
        } else {
            ctx.registerConfig(ModConfig.Type.SERVER, SERVER_CONFIG_SPEC.build());
        }
    }
    ...
    public static void registerRecipes(RegistryEvent.Register<IRecipe> event) {
        if (CONFIG.items.recipes.get()) {
            if (CONFIG.items.ingots.get()) {
    ...
    public void gatherData(GatherDataEvent event) {
        ...
        event.getGenerator().addProvider(event.includeServer(), (DataProvider.Factory<? extends DataProvider>)  output -> {


            return new LootTableProvider(output, Set.of(),
                List.of(IcbmLootTableProvider.INSTANCE))
        });
    ...
    @SubscribeEvent
    public static void onConfigChangedEvent(ModConfigEvent.Reloading event) {
        if (event.getConfig().getModId().equals(IcbmConstants.MOD_ID)) {
