        ...
        if (canReceive(sender, packet)) {
            ...
            if (packet instanceof ITriggerActionMessage) {
                ...
                if (sender instanceof FakeRadioSender) {
                    ...
                    host.setFiringPackage(new FiringPackage(new BasicTargetingModel(host.getTarget()), new EntityCause(((FakeRadioSender) sender).player), 0));
                } else {
                    // TODO set cause to radio
                    host.setFiringPackage(new FiringPackage(new BasicTargetingModel(host.getTarget()), null, 0));
                }
