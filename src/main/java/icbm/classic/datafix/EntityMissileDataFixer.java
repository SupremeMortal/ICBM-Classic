...
import icbm.classic.world.missile.logic.targeting.BallisticTargetingModel;
...
public class EntityMissileDataFixer implements IFixableData {
        ...
        if (existingSave.contains(ENTITY_ID) && existingSave.getString(ENTITY_ID).equalsIgnoreCase(ICBMClassicAPI.ID + ":missile")) {
            ...
            if (existingSave.contains("explosive")) {
                ...
                if (stackSave.contains("Damage")) {
                    final int damage = stackSave.getInt("Damage");

                    if (damage == ICBMExplosives.HYPERSONIC.getRegistryID()) {

                        // Change to sonic id
                        stackSave.putInt("Damage", ICBMExplosives.SONIC.getRegistryID());
    ...
    private void convertEntityMissileTags(CompoundTag existingSave) {
        ...
        // 3 -> homing
        // 4 -> dead_aim
        final int missileType = existingSave.getInt("missileType");
    ...
    private void convertExplosiveData(CompoundTag existingSave) {
        ...
        //explosiveID -> int
        final int explosiveId = existingSave.getInt("explosiveID");
    ...
    private void convertTargetData(CompoundTag existingSave, CompoundTag missile, int missileType) {
        if (existingSave.contains("target")) {
            ...
            targetData.putString("id", missileType == 0 ? BallisticTargetingModel.REG_NAME.toString() : BasicTargetingModel.REG_NAME.toString());

            // target -> compound with xyz
            data.putDouble("x", existingSave.getCompound("target").getDouble("x"));
            data.putDouble("y", existingSave.getCompound("target").getDouble("y"));
            data.putDouble("z", existingSave.getCompound("target").getDouble("z"));

            // targetHeight -> int
            if (missileType == 0) {
                data.putDouble("impact_height", existingSave.getInt("targetHeight"));
            }
        }
    }
    ...
    private void convertFlightLogic(CompoundTag existingSave, CompoundTag missile, int missileType) {
        ...
        if (missileType == 0) {
            ...
            int preLauncherTime = existingSave.getInt("preLaunchSmokeTimer");
            ...
            inputs.putDouble("start_x", existingSave.getTagList("Pos", 6).getDoubleAt(0));
            inputs.putDouble("start_y", existingSave.getTagList("Pos", 6).getDoubleAt(1));
            inputs.putDouble("start_z", existingSave.getTagList("Pos", 6).getDoubleAt(2));

            // target -> compound with xyz, using it for end
            inputs.putDouble("end_x", existingSave.getCompound("target").getDouble("x"));
            inputs.putDouble("end_y", existingSave.getCompound("target").getDouble("y"));
            inputs.putDouble("end_z", existingSave.getCompound("target").getDouble("z"));

            // lockHeight -> double
            // preLaunchSmokeTimer -> int
            final CompoundTag timers = new CompoundTag();
            data.put("timers", timers);
            timers.putInt("engine_warm_up", preLauncherTime);
            timers.putDouble("lock_height", lockHeight);
        }
        // Cruise
        else if (missileType == 1) {
            data.putInt("fuel", ConfigMissile.CRUISE_FUEL);
        }
        // RPG
        else if (missileType == 2) {
            data.putInt("fuel", ConfigMissile.HANDHELD_FUEL);
        }
    }
    ...
    private void convertMissileSource(CompoundTag existingSave, CompoundTag missile, int missileType) {
        ...
        if (missileType == 0) {
            ...
            pos.putDouble("x", existingSave.getCompound("launcherPos").getDouble("x"));
            pos.putDouble("y", existingSave.getCompound("launcherPos").getDouble("y"));
            pos.putDouble("z", existingSave.getCompound("launcherPos").getDouble("z"));
        ...
        // cruise missiles || rpg missile
            ...
            // launcherPos -> compound with x y z
            pos.putDouble("x", existingSave.getCompound("sourcePos").getDouble("x"));
            pos.putDouble("y", existingSave.getCompound("sourcePos").getDouble("y"));
            pos.putDouble("z", existingSave.getCompound("sourcePos").getDouble("z"));
    ...
    private void convertProjectileTags(CompoundTag existingSave) {
        ...
        tilePos.putInt("x", existingSave.getInt("xTilePos"));
        tilePos.putInt("y", existingSave.getInt("yTilePos"));
        tilePos.putInt("z", existingSave.getInt("zTilePos"));
        ground.put("pos", tilePos);

        ground.setByte("side", existingSave.getByte("sideTilePos"));
        final int oldBlockState = existingSave.getInt("inTileState");
        ...
        ticks.putInt("air", existingSave.getInt("ticksInAir"));
        ticks.putInt("ground", existingSave.getInt("life"));
    }
