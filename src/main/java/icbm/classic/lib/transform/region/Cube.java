...
public class Cube extends Shape3D implements Cloneable, IByteBufWriter {
    ...
    public List<BlockEntity> getTilesInArea(Level level) {
        ...
        for (ChunkAccess chunk : getChunks(world)) {
     ...
     * @return
     */
    public List<ChunkAccess> getChunks(Level level) {
     ...
     * @return list
     */
    public List<ChunkAccess> getChunks(Level level, boolean loaded) {
        List<ChunkAccess> chunks = new ArrayList();
        for (int chunkX = (min().xi() >> 4) - 1; chunkX <= (max().xi() >> 4) + 1; chunkX++) {
            for (int chunkZ = (min().zi() >> 4) - 1; chunkZ <= (max().zi() >> 4) + 1; chunkZ++) {
                if (loaded || (!(level instanceof ServerLevel) || level.hasChunk(chunkX, chunkZ))) {
